...
- **Req 01** — LLM Provider (search-term generation, classification, idea generation)
- **Req 02** — Search Provider (web search execution)
- **Req 03** — Keyword Enrichment Pipeline (shares `BasePipeline`; supplies the keyword output consumed for the same section)
- **Req 06** — State Management (pipeline state persistence and resumability)
...
- [ ] 2. Implement `read_keywords` node
  - Read all keywords grouped by Sub_Section
  - If the file is missing, empty, or cannot be parsed into Sub_Sections, log an error naming the cause and terminate without writing any output file
...
- [ ] 7. Implement zero-accepted guard
  - When no accepted results remain after the retry is exhausted, skip Idea_Record generation for that keyword, log the reason, and continue with the next keyword
  - _Requirements: 4.7_
