...
| built-in   | Integration with ARIA `StateStore` (Req 4) for tracking   |
...
class RetryPolicy(BaseModel):
    max_attempts: int = 3            # immediate retries per cycle (Req 8.1)
    backoff_multiplier: float = 2.0  # exponential factor (Req 8.1)
    initial_delay: float = 1.0       # first retry delay in seconds (Req 8.1)
    max_total_attempts: int = 5      # across immediate + queue cycles (Req 8.5)
    halt_on_failure: bool = True     # pause pipeline on escalation (Req 8.9)
```

### RunSummary (Pydantic)

```python
class RunSummary(BaseModel):         # Req 8.7
    total: int = 0
    successful: int = 0
    failed: int = 0
    retry_queued: int = 0
...
- **Search calls** — a fixed policy: exactly one retry after a 3-second delay
  ...
  failed and the task proceeds to the retry queue (Req 8.2).
- On final failure of a cycle, the decorator calls
  `state_store.mark_failed(task_id, error, attempt)` and enqueues the task.

## StateStore Integration (Req 4)

```python
state_store.mark_failed(task_id, error: str, attempt: int)
state_store.mark_retry_queued(task_id)        # status -> retry_queued (Req 8.3)
state_store.increment_attempts(task_id)       # persist cumulative count (Req 8.6)
state_store.mark_escalated(task_id)           # ERROR + pause (Req 8.9)
state_store.get_dependents(task_id)           # downstream lookup (Req 8.10)
state_store.mark_blocked(dependent_id)        # blocked_by_dependency (Req 8.10)
```

## Retry Queue & Cumulative Attempts (Req 8.3–8.6)

- A task that exhausts immediate retries is set to `retry_queued` and added to the
  queue — never discarded (Req 8.3).
- The queue is processed only **after all other pending tasks in the current batch
  complete** — never mid-batch (Req 8.4).
...
- A task with downstream dependents is re-attempted until success or until the
  cumulative count reaches `max_total_attempts` (Req 8.5).

## Escalation (Req 8.9, 8.10)
