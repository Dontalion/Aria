# Tasks — Req 14: Self-Aware Conversational Skill/Workflow Creation

## Dependencies

- Req 13 (Skills/Workflows) — schema, validation, on-disk format, and hot-reload
- Req 1 (LLM Provider) — drives the conversation and Markdown generation
- Req 16 (Dynamic Agents) — the registry also includes agents
...
- [ ] 4. Build registry-aware system prompts
  - Create `src/aria/chat/prompts.py`; embed the registry snapshot (tools, skills,
    workflows, agents) and the Req 13 schema rules into the system and generation
    prompts
  - _Requirements: 16.9_
...
- [ ] 8. Implement schema-validation retries
  - Validate generated Markdown with the Req 13 loaders; on failure regenerate up to
...
- [ ] 10. Implement save-and-notify on success
  - Write a valid file to the skills/ or workflows/ directory and notify the user of
    the created name and path (registration via Req 13 hot-reload)
  - _Requirements: 16.4, 16.5_
