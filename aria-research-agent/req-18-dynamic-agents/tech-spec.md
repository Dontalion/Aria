# Req 18 — Dynamic Agent Creation and Lifecycle — Technical Spec

## Architecture

Dynamic agents are produced by an `AgentFactory`, run by a dynamic runtime, and
tracked in the State_Store. The Orchestrator (Req 05) is the only component allowed to
...
task needs specialist → AgentFactory.propose() → present definition to user
   ...
   └─ rejected  → discard definition + notify (no activation)            (Req 18.2, 18.3)
```

## Models (Req 18.1, 18.5, 18.7)
...
class AgentConfig(BaseModel):
    max_concurrent_temporary: int = Field(5, ge=1)        # Req 18.4
    default_lifecycle: Literal["temporary", "persistent"] = "temporary"
    temporary_timeout_minutes: int = Field(30, ge=1)      # Req 18.8
    persistent_dir: str = "./agents"
```

## AgentFactory (Req 18.1–18.4) — `src/aria/agents/factory.py`

```python
class AgentFactory:
    def propose(self, task: Task, registry: CapabilityRegistry) -> AgentDefinition:
        # Only invoked when no existing agent in the registry covers the task (Req 18.1).
        defn = self._llm_design(task)                      # name/role/model/tools/prompt
        defn.tools = [t for t in defn.tools if t in self.user_granted_tools]  # Req 18.1
        return defn                                        # NOT yet activated

    async def create(self, defn: AgentDefinition, approved: bool) -> DynamicAgent:
        if not approved:                                   # Req 18.3 — discard + notify
        ...
        if defn.lifecycle == "temporary" and self.live_temporary() >= self.cfg.max_concurrent_temporary:
            raise TooManyTemporaryAgents(self.cfg.max_concurrent_temporary)  # Req 18.4
        agent = DynamicAgent(defn, tools=self._bind_tools(defn.tools))
        self.state.record_agent(defn)                      # Req 18.7 — audit row
        return agent
```
...
## Dynamic Runtime (Req 18.8) — `src/aria/agents/dynamic.py`

```python
class DynamicAgent:
    async def run(self, task: Task) -> Any:
        async with asyncio.timeout(self.cfg.temporary_timeout_minutes * 60):  # Req 18.8
            return await self._execute(task)               # tools limited to self.tools
    async def cleanup(self):                               # on task/pipeline end (Req 18.5)
        await self._release_resources()
```

On `TimeoutError`, the runtime destroys the agent, releases resources, marks the audit
row terminated, and notifies the user (Req 18.8).

## Lifecycle Management (Req 18.5, 18.6)
