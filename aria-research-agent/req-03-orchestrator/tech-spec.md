...
LangGraph `StateGraph` with a `TypedDict` state schema, async execution, and checkpointing wired to the State_Store (Req 4). Concurrency is bounded by an `asyncio.Semaphore` sized from config.
...
`pending_user_decision`, when set, describes the two ambiguous sub-sections and the explanation shown to the user (Req 3.9); it is cleared once the user resolves the choice (Req 3.11).
...
| `resolve_dependencies` | Classify each sub-section pair as independent / dependent / **ambiguous** (Req 3.8) |
| `await_user_decision` | Set `pending_user_decision`, halt scheduling of the pair, resume on user input (Req 3.9–3.11) |
| `execute_parallel` | Fan out ready tasks respecting `max_parallel` via `asyncio.Semaphore` (Req 3.1, 3.2, 3.5) |
| `check_dependencies` | Evaluate whether deferred tasks are now unblocked (Req 3.6, 3.7) |
...
## Concurrency Control (Req 3.1–3.5)

```python
sem = asyncio.Semaphore(max_parallel)   # max_parallel from Req 5 Config
...
## Ambiguous-Dependency Handling (Req 3.8–3.11)
...
## Configuration Validation (Req 3.3, 3.4)
...
Loaded via Req 5. A value `< 1` raises a config error naming the field and exits before any task runs.

## Checkpointing Integration

State persists through the State_Store (Req 4). On resume the orchestrator reloads task state, skips `completed` tasks, and re-runs any task left `in_progress` (reset to `pending` by the State_Store on startup).
...
- `langgraph >= 0.2`
- `asyncio` (stdlib)
- `pydantic` for config validation (`max_parallel` from Req 5 Config)
- Internal: LLM provider (Req 1), Search provider (Req 2), StateStore (Req 4)
