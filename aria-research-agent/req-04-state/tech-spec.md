...
| completed_at   | DateTime (nullable) | Set only when status → completed (Req 4.4)  |
...
class StateStore:
    ...
    async def load_on_startup(self) -> None: ...       # skip completed, in_progress → pending (Req 4.3)
    async def create_task(self, type, section, ...) -> Task: ...
    async def get_pending / get_failed / get_by_section / get_progress(...): ...  # Req 4.9
    async def get_ready_tasks(self) -> list[Task]: ... # pending + deps completed (Req 4.8)
    async def mark_in_progress / mark_completed / mark_failed(...): ...  # commit before next (Req 4.10, 4.4)
    async def enqueue_retry(self, task_id) -> None: ... # retry_queued, attempt_count += 1 (Req 4.7)
```

## Continuous Persistence (Req 4.10) — Key Behavior
...
- **Completion (Req 4.4, 4.5):** `mark_completed()` runs only after output is
  written to the target file AND confirmed present; sets `completed_at`. A
  Section is completed when all its tasks are completed.
- **Retry queue (Req 4.7):** `enqueue_retry()` sets `retry_queued` and increments
  `attempt_count` by 1, up to the configured max (default 5); then stays `failed`
  with `last_error` set.
- **Ready tasks (Req 4.8):** `get_ready_tasks()` returns pending tasks whose
  dependency UUIDs are all `completed`; others stay deferred.
- **Unsatisfiable deps (Req 4.11):** detect cycles (DFS / topological sort) and
...
- `sqlalchemy[asyncio] >= 2.0`, `aiosqlite` (default), `asyncpg` (optional), `alembic`; internal: Config (Req 5) DB path/URL, Data Layer (Req 11) relational store
