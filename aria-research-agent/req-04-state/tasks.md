...
- **Req 5** — Config (database path/URL and `max_attempts` come from the config system)
- **Req 11** — Data Layer (relational store engine/session foundation this layer builds on)
...
- [ ] 5. Implement verified completion
  ...
  - Mark a Section completed when all of its tasks are completed
