# Req 16 — Self-Aware Conversational Skill/Workflow Creation — Technical Spec

## Architecture

A Typer subcommand (`aria chat`) launches a multi-turn session. The `ChatSession`
drives the conversation through the LLM provider (Req 01), injecting a
...
produced by the `Generator`, validated against the Req 15 schema, then saved directly
to the skills/workflows directory.

## Capability Registry (Req 16.9, 16.10)
...
class CapabilityRegistry:
    ...
    def describe(self) -> dict:                      # {tools, skills, workflows, agents}
        ...                                          # used for self-description (Req 16.10)
    ...
    def installed_tools(self) -> set[str]: ...       # for tool-availability check (Req 16.8)
    def refresh(self) -> None: ...                   # re-read after add/modify/delete
```

The registry aggregates the Req 15 skill/workflow loaders, the Req 18 agent registry,
and the installed tool list. `describe()` returns a structured summary; `find_*` backs
the pre-generation existence check (Req 16.2, 16.5).

## Chat Session (Req 16.1, 16.2, 16.3, 16.5, 16.6, 16.10)
...
class ChatSession:
    ...
    async def handle_describe_request(self, intent) -> str:   # Req 16.10 self-summary
    async def handle_create_request(self, intent) -> None:
        ...
        if existing:                                 # Req 16.3 / 16.5
            choice = await self.prompt_choice(["use-as-is", "modify", "variant"])
            return await self.apply_choice(choice, existing, intent)
        await self.create_new(intent)                # Req 16.4 / 16.5
    async def refine(self, target_file: Path, change) -> None: ...   # Req 16.6
```
...
and referenceable tools (Req 16.10).## Generator (Req 16.4, 16.7, 16.8)

```python
# src/aria/chat/generator.py
class Generator:
    MAX_RETRIES = 2                                  # 2 *additional* attempts (Req 16.7)
    async def generate(self, kind: Literal["skill", "workflow"], intent, registry):
        for attempt in range(self.MAX_RETRIES + 1):
            ...
            errors = validate_markdown(kind, md)     # Req 15 schema
            if errors:
                continue                             # regenerate
            ...
            return md
        raise GenerationFailed(errors)               # Req 16.7 — error, save nothing
```

- **Schema retries (Req 16.7):** validation uses the Req 15 loaders. Failure triggers up
  to 2 additional regenerations; if it still fails, raise and write no file.
- **Tool gate (Req 16.8):** any referenced tool absent from `registry.installed_tools()`
  rejects the file naming the tool; nothing is saved.
- **Save + notify (Req 16.4):** a valid file is written to `skills/` or `workflows/` and
  the user is told the name and path. Saving relies on Req 15's hot-reload to register it.

## System Prompt (Req 16.9)
...
descriptions, agents) plus the Req 15 schema rules, so both the existence check and
...
- Req 15 (Skills/Workflows) — schema, validation, on-disk format, hot-reload.
- Req 01 (LLM Provider) — conversation and Markdown generation.
- Req 18 (Dynamic Agents) — registry includes agents for self-description + checks.
