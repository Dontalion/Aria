...
- **Req 01** — LLM Provider (called inside execute nodes for generation/classification)
- **Req 02** — Search Provider (called inside execute nodes for web search)
- **Req 06** — State Management (StateStore checkpoint persistence; resume/skip logic)
- **Req 07** — Config (`ExecutionConfig.max_parallel`, validated at load time)
...
- [ ] 3. Implement `resolve_dependencies` node
  - For each pair of Sub_Sections, classify the relationship as `independent`, `dependent`, or `ambiguous`
...
- [ ] 5. Implement `execute_parallel` node with `asyncio.Semaphore` concurrency cap
  - Create `sem = asyncio.Semaphore(max_parallel)` sized from `ExecutionConfig` (Req 07)
...
- [ ] 11. Integrate checkpointing with the StateStore (resume logic)
  - Persist task state through the StateStore (Req 06); on resume, reload task state, skip `completed` tasks, and re-run any task left `in_progress` (reset to `pending` by the StateStore on startup)
  - _Requirements: 5.1_
