# Req 12 — Adversarial Review Agent — Technical Spec

## Architecture

The reviewer is an agent backed by its own `LLMProvider` (Req 1), wired into the
orchestration graph (Req 3) as a routing node placed **before** a task is marked
...
class ReviewResult(BaseModel):
    ...
    def _feedback_required(self):
        if self.classification in ("revise", "reject") and not self.feedback.strip():
            raise ValueError("feedback must be non-empty for revise/reject")   # Req 12.4
        return self

class ReviewConfig(BaseModel):
    enabled: bool = False                          # Req 12.1 — disabled by default
    model: str | None = None
    max_revisions: int = Field(2, ge=0, le=5)      # Req 12.9
    retry_limit: int = Field(2, ge=0, le=10)

    @model_validator(mode="after")
    def _validate_when_enabled(self):              # Req 12.12
...
(Req 12.3); executor and reviewer use separate `LLMConfig` blocks.

## Routing Node (Req 12.2, 12.5–12.7, 12.10, 12.11)
...
async def review_node(state):
    ...
    for _ in range(reviewer.cfg.retry_limit + 1):
        try:
            ...
            continue                               # Req 12.10 — retry
        except ParseError as e:                    # Req 12.11 — invalid response → REVISE
    ...
    state["review"], state["unreviewed"] = None, True   # Req 12.10 — retain, not discard
    return state
...
def route_review(state):
    ...
    if r.classification == "revise" and state["revision_count"] < state["max_revisions"]:
        state["revision_count"] += 1               # Req 12.5 — increment + feed back
        return "revise"
    return "discard"                               # Req 12.6 (revise≥max) / 14.7 (reject)
```

## Structural Gate — ACCEPT Is Quality-Only (Req 12.8, Key Behavior)
...
def structural_gate(item, output_file, schema) -> bool:
    ...
    if output_file.has_title(item["title"]):       # idea-generation workflow (see `workflows/idea-generation.md`) — no duplicate title (case-insensitive)
        log.info("discarded ACCEPT item: duplicate title %r", item["title"]); return False
    try:
        schema(**item)                             # Req 10 — WorkflowOutput conformance
...
- Req 1 (LLM Provider) — second model instance for the reviewer.
- Req 3 (Orchestrator) — review routing node and feedback edge to the Executor.
- Req 5 (Configuration) — `review` settings and their enabled-time validation.
