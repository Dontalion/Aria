# Tasks — Req 12: Adversarial Review Agent
...
- Req 5 (Configuration) — `review` settings and enabled-time validation
...
- [ ] 8. Implement the structural gate after ACCEPT (key behavior)
  - Treat ACCEPT as quality-only; finalize only if the item also has no duplicate
    title (idea-generation workflow (see `workflows/idea-generation.md`)) and conforms to the WorkflowOutput schema (Req 10)
