...
- **Req 1** — LLM Provider (search-term generation, classification, keyword generation)
- **Req 2** — Search Provider (web search calls)
- **Req 4** — State Management (graph state persistence and resumability)
...
- [ ] 3. Implement `read_keywords` node
  - Parse source file, collect keywords under the target Section into `KeywordInput` models
  - If the Section is absent or empty, log an error naming it and terminate that Section without creating or modifying any output file
...
- [ ] 9. Implement `generate_keywords` node
  - From ≥1 accepted result, produce exactly 10 new keywords, each distinct from source keywords and from keywords already generated in the same Section
  - _Requirements: 3.8_
