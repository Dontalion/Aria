# Req 15 — Markdown-Defined Skills and Workflows — Technical Spec
...
Req 07). At startup the loaders read **every** `.md` file in each directory, parse
...
```
config (Req 07) → SkillLoader.load_all() → {name: SkillDefinition}
...
class SkillDefinition(BaseModel):
    name: str                       # non-empty (Req 15.2)
    description: str                # non-empty (Req 15.2)
...
class WorkflowDefinition(BaseModel):
    name: str                       # non-empty (Req 15.4)
    description: str
    steps: list[WorkflowStep]       # ordered, length ≥ 1 (Req 15.4)
```

## Loaders (Req 15.1, 15.3)
...
- A workflow with zero steps, a step referencing an unknown skill name (Req 15.6), or
  a cycle in `depends_on` (detected via topological sort / DFS) is invalid.
- **Fail-fast (Req 15.7):** if **any** file fails validation, the loader raises and ARIA
...
- Req 07 (Configuration) — supplies the skills/workflows directory paths and their
