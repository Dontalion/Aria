# Tasks — Req 07: Configuration Management
...
- [ ] 4. Implement `${ENV_VAR}` interpolation
  - Substitute `${ENV_VAR_NAME}` references with environment variable values during load
  - When a referenced variable is unset, raise a field-level error naming the variable and exit non-zero before any research
...
- [ ] 6. Add documented default config file
...
- [ ] 7. Write unit tests
  ...
  - Test `${ENV_VAR}` interpolation for set and unset variables
