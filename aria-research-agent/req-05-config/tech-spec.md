...
Pydantic enforces every allowed set and numeric bound at load time (Req 5.2, 5.7):
...
class LLMConfig(BaseModel):
    provider: Literal["openrouter", "ollama", "cli_tool", "direct_api"]  # Req 5.2
    model: str | None = None
    base_url: str | None = None

class SearchConfig(BaseModel):
    provider: Literal["duckduckgo", "tavily", "brave"] = "duckduckgo"   # Req 5.2
    max_results: int = Field(5, ge=1, le=20)

class ExecutionConfig(BaseModel):
    max_parallel: int = Field(3, ge=1, le=10)      # Req 5.2: integer 1–10, default 3
    retry_limit: int = Field(2, ge=0, le=5)        # Req 5.2: integer 0–5
...
non-zero before any research starts (Req 5.7).
...
def load_config(path: Path | None = None) -> ConfigSchema:
    ...
    if not resolved.exists():
        return ConfigSchema.model_validate({            # Req 5.6: defaults, no termination
            "llm": {"provider": "openrouter"}
        })
    raw = yaml.safe_load(resolved.read_text()) or {}
    try:
        return ConfigSchema.model_validate(raw)         # Req 5.3 / 5.5 / 5.7
...
resolve path (--config flag → default config.yaml)
   → file missing? → build ConfigSchema from documented defaults (Req 5.6)
   → file present? → parse YAML
       → Pydantic validate (missing required / out-of-bounds → ConfigError) (Req 5.3, 5.7)
   → return validated ConfigSchema
   → instantiate provider clients
       → each client resolves its env var (unset → ConfigError) (Req 5.8)
...
any research starts (Req 5.3, 5.7, 5.8).
