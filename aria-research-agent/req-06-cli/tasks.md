# Tasks — Req 6: CLI Interface

## Dependencies
- Req 3 (Orchestrator) — `run`/`run-all` dispatch pipelines and honor the shutdown flag
- Req 4 (State) — `status`/`reset` read and clear persisted state; continuous persistence and resume
- Req 5 (Config) — loads config, resolves section identifiers and `--config` path
...
- [ ] 7. Implement `chat` subcommand (stub)
  - Placeholder for conversational skill/workflow creation (Req 14)
  - _Requirements: 8.1_
